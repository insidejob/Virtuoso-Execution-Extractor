...
/**
 * Comprehensive Extraction v4 - With proper selector hierarchy
 * 
 * Selector Priority:
 * 1. HINT (from GUESS selector) - Always preferred
 * 2. Explicit selectors as fallback (XPath, CSS, ID, etc.)
 * 3. NEVER use element aliases directly
 * 
 * Improvements over v3:
 * - Proper HINT extraction and usage
 * - Element alias handling
...
class ComprehensiveExtractorV4 {
    constructor() {
        this.config = {
        ...
        this.elementAliases = new Map(); // Track element aliases
    }
    
    async extract(url) {
        console.log('🚀 Comprehensive Extraction v4 Starting\n');
        ...
        try {
            ...
            // Extract element aliases if present
            ...
            // Step 4: Extract and save NLP with proper selector hierarchy
            console.log('\n📋 Step 4: Converting to NLP (v4 with HINT priority)...');
            ...
            // Step 5: Extract and save variables
            ...
            // Final summary
            console.log('\n' + '=' .repeat(70));
            console.log('✅ EXTRACTION COMPLETE (v4)\n');
            console.log(`📁 Output Directory: ${folderPath}`);
            console.log('\n📊 Files Created:');
            console.log('  ├── execution.nlp.txt          (HINT-prioritized NLP)');
    ...
    stepToNLP(step) {
        ...
        switch (step.action) {
            ...
            case 'CLICK':
                const clickTarget = this.getSelectorDescription(selectors);
                return `Click on ${clickTarget}`;
                
            case 'PICK':
            case 'SELECT':
                if (selectors.hint) {
                    return `Pick "${step.value}" from dropdown "${selectors.hint}"`;
                }
                return `Select "${step.value}"`;
                
            case 'WAIT_FOR_ELEMENT':
                const waitTarget = this.getSelectorDescription(selectors);
                return `Wait for ${waitTarget}`;
                
            case 'ASSERT_EXISTS':
            case 'ASSERT':
                // Special handling for large text
                if (selectors.text && selectors.text.length > 100) {
                    ...
                    return `Look for element ${assertTarget} on page`;
                }
                
                // Handle variable references in GUESS
                if (selectors.guessVariable) {
                    return `Look for element $${selectors.guessVariable} on page`;
                }
            ...
            case 'MOUSE':
                ...
                if (mouseAction === 'CLICK') {
                    return `Mouse click ${mouseTarget}`;
    ...
    extractSelectors(step) {
        const selectors = {
            hint: null,          // HINT from GUESS (highest priority)
            guessVariable: null, // Variable from GUESS
            xpath: null,         // XPath selector
            xpathId: null,       // XPath with ID
            xpathText: null,     // XPath with text
            xpathAttribute: null,// XPath with attribute
            css: null,           // CSS selector
            id: null,            // ID selector
            jsPath: null,        // JS Path selector
    ...
    extractSelectorsFromAlias(aliasDefinition) {
        // Extract selectors from alias definition
        // This would need to match the structure of how aliases are stored
        const selectors = {
            hint: null,
            xpath: null,
            css: null,
    ...
    getSelectorDescription(selectors) {
        // Priority order (as per user specification):
        // 1. HINT (from GUESS)
        // 2. Text (if reasonable size)
        // 3. ID selector (preferred over XPath for readability)
        // 4. XPath variants
        // 5. CSS
        // 6. JS Path
        // 7. ERROR if no selectors found
        ...
        // ID selectors are good - they're readable and maintainable
        if (selectors.id) {
            return `"${selectors.id}"`;
        }
        
        // XPath variants as fallback
        ...
        // ERROR: No selectors found - this should never happen
        console.error('⚠️ ERROR: No selectors found for element. This needs debugging.');
    ...
    createMetadata(ids, journeyData, executionData, goalData, projectData) {
        return {
            ...
            improvements: {
                v4: [
                    'HINT selector priority',
                    'Element alias resolution',
                    'Full selector hierarchy support',
                    'XPath variants (ID, Text, Attribute)',
    ...
    createSummaryReport(folderPath, nlp, variables, metadata) {
        ...
        lines.push('# 📊 Extraction Summary Report v4');
        lines.push('');
        ...
        lines.push(`- **Element Aliases**: ${metadata.journey.element_aliases}`);
        lines.push('');
        ...
        lines.push('## 🎯 v4 Selector Hierarchy');
        lines.push('1. **HINT** (from GUESS) - Always preferred');
        lines.push('2. **Text** (if ≤100 chars)');
        lines.push('3. **ID** selector');
        lines.push('4. **XPath** variants (ID, Text, Attribute, Standard)');
        lines.push('5. **CSS** selector');
        lines.push('6. **JS Path**');
        lines.push('7. **"element"** (last resort)');
        lines.push('');
        
        lines.push('## ✅ v4 Improvements');
        lines.push('- HINT selector prioritization');
        lines.push('- Element alias resolution');
        lines.push('- Full selector type support');
        lines.push('- Proper fallback chain');
        lines.push('');
        
        lines.push('## 📁 Files Generated');
        lines.push('```');
        lines.push('.');
        lines.push('├── execution.nlp.txt          # HINT-prioritized NLP');
        lines.push('├── variables-used.json        # All used variables');
...
if (require.main === module) {
    ...
    if (args.length === 0) {
        console.log('Usage: node comprehensive-extraction-v4.js <URL>');
        console.log('Example: node comprehensive-extraction-v4.js "https://app2.virtuoso.qa/#/project/4889/execution/86332/journey/527256"');
        process.exit(1);
    }
    
    const extractor = new ComprehensiveExtractorV4();
